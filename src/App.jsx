...
// Main Todo App Component (now simplified)
const TodoApp = () => {
  const { 
    actions, 
    activeId, 
  ...
  const handleDragOver = (event) => {
    ...
    // Check if we're over a list (not a task)
  ...
  const handleDragEnd = (event) => {
    ...
    // If we're reordering within the same list
  ...
  return (
    ...
    >
      <div className='min-h-screen bg-gradient-to-br from-purple-400 via-purple-500 to-purple-600 relative overflow-hidden'>
        ...
        <div className='absolute z-30 top-[20px] left-[50%] translate-x-[-50%] lg:left-[280px] lg:translate-x-0 w-24 h-24 sm:w-32 sm:h-32 bg-cyan-400 rounded-full md:block hidden'></div>
        <div className='absolute bottom-0 left-0 w-32 h-32 sm:w-48 sm:h-48 lg:w-64 lg:h-64 bg-pink-400 rounded-full opacity-60 -translate-x-16 sm:-translate-x-24 lg:-translate-x-32 translate-y-16 sm:translate-y-24 lg:translate-y-32'></div>
        <div className='absolute bottom-10 sm:bottom-20 right-0 w-24 h-24 sm:w-32 sm:h-32 lg:w-40 lg:h-40 bg-yellow-400 rounded-full opacity-70 translate-x-12 sm:translate-x-16 lg:translate-x-20'></div>
        ...
        <div className='pt-[50px] sm:pt-[60px] lg:pt-[70px] container mx-auto p-3 sm:p-4 lg:p-6 relative z-10 max-w-7xl'>
          {/* Main content wrapper - responsive flex direction */}
          <div className='rounded-2xl shadow-lg flex flex-col lg:flex-row border-2 sm:border-4 border-white relative min-h-[calc(100vh-100px)] sm:min-h-[calc(100vh-120px)] lg:min-h-[calc(100vh-140px)]'>
            ...
            {/* Main content - responsive flex grow */}
        ...
        {/* Drag Overlay - responsive sizing */}
        <DragOverlay>
          {activeTask ? (
            <div className='flex items-center gap-2 sm:gap-4 py-2 sm:py-3 px-3 sm:px-4 bg-white rounded-xl sm:rounded-2xl border border-gray-100 shadow-lg opacity-90 max-w-[90vw] sm:max-w-none'>
              ...
              <div className='flex-1 min-w-0'>
                <p className={`${activeTask.completed ? 'line-through text-gray-400' : 'text-gray-700'} font-medium text-sm sm:text-base truncate`}>
                  {activeTask.text}
                </p>
              </div>
