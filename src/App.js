...
import { Plus, Search, Calendar, Settings, Filter } from 'lucide-react';

const TodoApp = () => {
  const [tasks, setTasks] = useState([
    { id: '1', text: 'Work out', completed: false, time: '8:00 am', category: 'personal' },
    { id: '2', text: 'Design team meeting', completed: false, time: '2:30 pm', category: 'work' },
    { id: '3', text: 'Hand off the project', completed: false, time: '7:00 pm', category: 'freelance' },
    { id: '4', text: 'Read 5 pages of "Sprint"', completed: false, time: '10:30 pm', category: 'personal' }
  ]);
      ...
      {/* <div className="absolute top-[-20px] left-[220px] z-30 w-32 h-32 bg-cyan-400 rounded-full"></div> */}
            ...
            <div className="space-y-1 mb-6">
              {categories.map(category => (
                <div key={category.id} className="flex items-center gap-3 py-2 px-3 rounded-lg hover:bg-gray-50 cursor-pointer">
                  <div 
                    className="w-3 h-3 rounded-full" 
                    style={{ backgroundColor: category.color }}
                  ></div>
                  <span className="text-gray-700 flex-1">{category.name}</span>
                  <span className="text-gray-400 text-sm">{getTaskCountByCategory(category.id)}</span>
                </div>
              ))}
          ...
          <div className="space-y-4">
            {tasks.map(task => (
              <div key={task.id} className="flex items-center gap-4 py-2 px-4 bg-white rounded-2xl border border-gray-100 hover:shadow-md transition-shadow">
                <div 
                ...
                <div className="flex-1">
                  <p className={`${task.completed ? 'line-through text-gray-400' : 'text-gray-500'} font-bold  `}>
                    {task.text}
                  </p>
                </div>
                
                <span className="text-sm text-gray-500 font-bold flex-shrink-0">{task.time}</span>
