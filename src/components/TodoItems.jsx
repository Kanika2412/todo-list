...
const TodoItems = ({ list, isDroppable = false }) => {
    ...
    return (
      <div
        className={`group flex items-center gap-2 sm:gap-3 py-2 sm:py-3 px-2 sm:px-3 rounded-lg hover:bg-gray-50 cursor-pointer relative transition-all ${
          ...
          overId === list.id
            ? 'bg-purple-100 border-2 border-purple-400 border-dashed scale-[1.02] sm:scale-105'
            : ''
        }`}
      ...
      >
        ...
        {editingListId === list.id ? (
          ...
          <span className='text-gray-700 flex-1 text-sm sm:text-base truncate min-w-0'>
            {list.name}
          </span>
        )}
